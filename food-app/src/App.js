...
function App() {
  ...
  const toggleDiets = (diet) => {
    // Tar in en matpreferens
    setSelectedDiets(
      (
        prev // Kopia av den gamla arrayen selectedDiets
      ) =>
  ...
  function sortByTime() {
    ...
    setSearchResult(
      [...searchResult].sort(
        (a, b) => a.readyInMinutes - b.readyInMinutes //Stigande
      )
    );
  }
  ...
  function sortByRating() {
    setRecipes(
      [...recipes].sort(
        (a, b) => b.rating - a.rating //Fallande
      )
    );
  }
  ...
  return (
    <div className="App mt-4">
      ...
      <div>
        <CheckboxList selected={selectedDiets} onToggle={toggleDiets} />
      </div>

      <div className="container mt-3">
        <div className="row justify-content-center g-2">
          <div className="col-auto">
            <Button text="Sort by time" onClick={() => sortByTime()} />
          </div>
          <div className="col-auto">
            <Button text="Sort by rating" onClick={() => sortByRating()} />
          </div>
        </div>
      </div>
